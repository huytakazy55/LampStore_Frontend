    ...
    >
      <div className='flex gap-3 h-full'>
        ...
        <div className='h-full aspect-square flex-shrink-0 bg-gradient-to-br from-gray-50 to-white flex items-center justify-center p-3 relative overflow-hidden'>
          {/* Discount Badge */}
          {hasDiscount && (
            <div className="absolute top-2 left-2 z-10 bg-gradient-to-r from-primary-500 to-secondary-500 text-white text-[9px] font-bold px-2 py-0.5 rounded-sm shadow-md">
              -{discountPercent}%
            </div>
          )}
          {imageSrc ? (
        ...
        <div className='flex flex-col flex-1 py-3 pr-3 min-w-0'>
          ...
          <div className='text-[10px] md:text-xs text-gray-400 flex items-center gap-1 mb-1.5'>
            <i className='bx bx-purchase-tag text-primary-500 text-xs'></i>
            Đã bán {product.sellCount || 0}
          </div>

          {/* Title */}
          <h3 className='text-xs md:text-sm font-semibold text-gray-800 line-clamp-2 leading-snug group-hover:text-primary-700 transition-colors duration-200'>
            {product.name}
          </h3>
          ...
          <div className='mt-auto flex items-end justify-between'>
            ...
            <button
              className='w-7 h-7 md:w-8 md:h-8 rounded-sm bg-primary-50 text-primary-600 flex items-center justify-center transition-all duration-300 group-hover:bg-primary-500 group-hover:text-white group-hover:shadow-lg group-hover:shadow-primary-200 group-hover:scale-105 active:scale-95'
...
const SectionProductCardCarousel = () =>
{
  ...
  return (
    <div className='w-full overflow-visible xl:mx-auto xl:max-w-[1440px] px-4 xl:px-0 mb-8'>
      <div className='border-b border-gray-300 pb-1 relative mb-6 md:mb-8 after:w-[30%] md:after:w-[16%] after:h-[1px] after:bg-secondary-400 after:absolute after:bottom-0 after:left-0'>
        <h3 className='font-medium text-sm md:text-h3 text-black flex items-center gap-2'>
          <i className='bx bx-trending-up text-secondary-500'></i>
          Sản phẩm bán chạy
        </h3>
      </div>
      <div>
        {loading ? (
          <div className='flex items-center justify-center h-[300px]'>
            <i className='bx bx-loader-alt bx-spin text-3xl text-secondary-400'></i>
          </div>
        ) : (
          <Slider2 {...settings}>
