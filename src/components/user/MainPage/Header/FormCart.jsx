...
const FormCart = ({toggleCart, popupRef}) => {
  return (
    <div ref={popupRef} onClick={(e) => e.stopPropagation()} className={`cart-modal w-[40rem] h-96 absolute shadow-sm -right-[5.7rem] top-14 z-[1000] border-t-2 bg-white border-yellow-400 text-white transition-all duration-300 ease-in-out ${toggleCart ? 'visible opacity-100 translate-y-0' : 'invisible opacity-0 translate-y-2'}`} id='FormCart'>
        <div className='w-full h-4/5 p-4 overflow-y-scroll'>
            <div className='flex justify-between items-center w-full h-28 border-b-[1px] border-gray-200 py-2 last:border-b-0'>
        ...
        <div className='h-1/5 flex justify-end items-center gap-6 p-4 border-t-gray-300 border-[1px]'>
            <div className='py-2 px-6 rounded-[5px] text-normal bg-gray-400 hover:bg-black hover:text-white'>View cart</div>
            <div className='py-2 px-6 rounded-[5px] text-normal bg-yellow-400 hover:bg-black hover:text-white'>Checkout</div>
        </div>
    </div>
  )
}
