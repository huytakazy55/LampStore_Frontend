...
const FormCart = ({ toggleCart, popupRef, setToggleCart }) =>
{
    ...
    return (
        <div
            ...
            className={`cart-modal w-[28rem] max-h-[32rem] absolute shadow-2xl -right-[4rem] top-14 z-[1000] border-t-4 bg-white border-rose-600 rounded-b-lg flex flex-col transition-all duration-300 ease-out origin-top-right ${toggleCart ? 'visible opacity-100 scale-100 translate-y-0' : 'invisible opacity-0 scale-95 translate-y-2'
                }`}
            id='FormCart'
        >
            <div className='p-4 border-b border-gray-100 flex justify-between items-center'>
                <h3 className='text-lg font-semibold text-gray-800'>Giỏ hàng của bạn ({cartItems.length})</h3>
                {cartItems.length > 0 && (
                    ...
                    cartItems.map((item) =>
                    {
                        ...
                        return (
                            <div key={item.key} className='flex justify-between items-start w-full group'>
                                <div
                                    className='w-20 h-20 bg-gray-50 rounded-md border border-gray-100 flex items-center justify-center overflow-hidden flex-shrink-0 cursor-pointer'
                                ...
                                <div className='flex-1 ml-4'>
                                    <span
                                        onClick={() => handleViewProduct(item.productId)}
                                        className='leading-snug text-sm font-medium text-gray-800 hover:text-rose-600 line-clamp-2 mb-1 transition-colors cursor-pointer block'
                                    >
                                        {item.name}
                                    </span>
                                    {optionText && (
                                        <div className='text-xs text-gray-500 mb-2'>Phân loại: {optionText}</div>
                                    )}
                                    <div className='flex justify-between items-center'>
                                        ...
                                        <div className='flex items-center border border-gray-200 rounded overflow-hidden'>
                                            <button
                                                ...
                                                className='w-6 h-6 flex items-center justify-center text-gray-500 hover:bg-gray-100 text-xs'
                                                disabled={item.quantity <= 1}
                                            >-</button>
                                            <span className='w-8 h-6 flex items-center justify-center text-xs border-x border-gray-200'>{item.quantity}</span>
                                            <button
                                                onClick={() => updateQuantity(item.key, item.quantity + 1)}
                                                className='w-6 h-6 flex items-center justify-center text-gray-500 hover:bg-gray-100 text-xs'
                                            >+</button>
